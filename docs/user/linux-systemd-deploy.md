...
默认配置文件已存在时，脚本会保留原文件，不会覆盖端口、日志路径、iLink 地址等用户改动。升级时如果旧配置缺少 `[telemetry]`、`[redis]` 或 `[audit]` section，脚本会非破坏性追加默认 telemetry、Redis 和 audit 配置，方便后续启用链路追踪、发送保护或发送审计。脚本不会删除或修改 `~/.webot-msg/config/auth.json`。
...
进入 systemd 服务的控制台时，直接运行同一个用户下的 `webot-msg`：

```bash
webot-msg
```

程序会读取 `~/.webot-msg/config/webot-msg.toml`。如果配置里的 control socket 已有运行中服务，`webot-msg` 会接入已有服务的控制台，不会再启动第二个实例；如果没有可用 socket，它才会启动新的前台 service。

也可以用第三方 socket 客户端连接正在运行服务暴露的本地 socket：
...
在 TTY 里用 `webot-msg` 接入已有服务时支持按键级 Tab 补全。通过 `socat` / `nc -U` 连接 socket 时按普通 line mode 输入命令，不提供按键级 Tab 补全。
...
升级不会覆盖已有 `~/.webot-msg/config/webot-msg.toml` 的既有字段，也不会把原本关闭的发送保护、发送审计或 telemetry 自动打开。升级到包含消息 ID 的版本后，所有普通文本正文最底部都会新增一行 uuid v7；如果你有自动化接收方依赖旧正文精确匹配，需要先让接收端容忍或剥离末行 uuid v7。HTTP API 响应 JSON 不新增 ID 字段。

已经通过 `/protection enable` 成功开启过的实例，会在升级重启后读取 `~/.webot-msg/state/protection.json` 并尝试一次自动恢复保护；已经通过 `/audit enable` 成功开启过的实例，会读取 `~/.webot-msg/state/audit.json` 并尝试一次自动恢复审计。如果 Redis 不可用，服务仍会启动，但对应能力保持关闭，并在日志中提示可修复后手动执行 `/protection enable` 或 `/audit enable`。如果旧配置缺少 `[telemetry]` section，脚本会追加 endpoint 为空的默认关闭配置；需要启用链路追踪时再填入 OTLP endpoint 和认证配置。首次开启保护或审计时，如果旧配置缺少 `[redis]` section，脚本会追加默认 Redis 配置；确认 Redis 地址和密码后，连接 control socket 执行 `/protection enable` 或 `/audit enable`。缺少 `[audit]` section 时，脚本会追加默认 `24h` TTL 配置。
...
Runtime config 只保存启动参数，不要写入 bot token、API token、context token 或消息正文。开启发送保护或发送审计时，`redis.password` 属于本机凭据；启用 telemetry 时，`telemetry.headers` 和 `telemetry.resource_attributes` 也可能包含 APM 鉴权信息。审计开启后，Redis 的 `audit:body` key 会保存完整发送正文，应按数据保留策略调整 `audit.body_ttl`。这些真实值都不要提交到 Git。
