...
pitch: 在本地登录微信 bot 后，用控制台或受保护 API 回复最近会话，并可通过 TOML 管理本地运行参数
status: current
last_reviewed: 2026-06-10
implemented_by:
  - architecture-overview
tags: [bot, messaging, local-api, config]
---
...
bot 登录态、消息上下文、发送入口和本地运行参数分散处理时，很容易把凭证、会话上下文、调试动作和部署差异混在一起。这个项目把它们收束到一个本地工具里，让开发者可以先登录 bot、等到消息上下文就绪，再从控制台或受保护 API 发出回复；运行参数则通过独立 TOML 配置表达，不混入凭据文件。
