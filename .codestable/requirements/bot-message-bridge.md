...
pitch: 在本地登录微信 bot 后，用控制台或受保护 API 回复最近会话，保护冻结期 API 消息可等待恢复后按序补发，并可通过 TOML、发送保护、审计和 systemd 脚本管理本地运行
status: current
last_reviewed: 2026-06-25
implemented_by:
  - architecture-overview
tags: [bot, messaging, local-api, config, deploy, systemd, console, autocomplete, protection, redis, queue, fifo, audit, uuid]
---
...
- 作为把 bot 接到自动化流程的人，我希望保护冻结期的 API 文本请求先被本地服务接收并在恢复后按顺序补发，而不是自己维护重试队列和排序。
...
bot 登录态、消息上下文、发送入口、本地运行参数、微信侧发送限制和 Linux 部署动作分散处理时，很容易把凭证、会话上下文、调试动作、保护边界和部署差异混在一起。这个项目把它们收束到一个本地工具里，让开发者可以先登录 bot、等到消息上下文就绪，再从控制台或受保护 API 发出回复；运行参数和 Redis 连接通过独立 TOML 配置表达，Linux systemd 部署通过仓库脚本编排，不混入凭据文件。对需要长期自动化发送的场景，用户可以从控制台开启 Redis 支撑的保护模式，把微信侧“需要主动对话”的限制变成明确提醒、冻结和 API 等待队列，而不是让每个外部调用者各自处理重试、排序和重复发送。
...
发送保护模式默认关闭，开启状态不写入 TOML，而是由服务进程写入 `~/.webot-msg/state/protection.json`。用户配置 Redis 后，在运行中控制台执行 `/protection enable` 开启；工具会在重启或升级后读取状态文件并尝试一次自动恢复保护。工具按 bot 记录最近主动对话后的普通文本和保护提醒下发次数，以及 24h 主动对话窗口。保护开启且普通文本预留成功时，工具会在用户请求正文末尾追加一行保护状态，显示本次发送计入后的剩余可发条数和距离限制触发的剩余时间；该状态来自同一次 Redis 预留，不需要外部调用者额外查询。接近次数限制或时间窗口限制时，工具会给当前上下文发送保护提醒并冻结普通文本发送；冻结期间，HTTP API 普通文本请求会被接收为待发队列并返回已入队，队列满时返回队满错误，控制台普通文本仍即时拒绝。用户从微信 App 主动给机器人发一条消息后，工具解除冻结、重置该 bot 的计数和窗口，并按先入先出的顺序补发队列里的 API 文本。用户可通过 `/protection status` 查看当前 active bot 离触发限制还剩多少次数或时间、以及还有多少待发 API 文本，通过 `/protection disable` 关闭运行期保护。
...
- API 发送队列只承接保护冻结期间的 HTTP 普通文本；控制台普通文本、typing、保护提醒和普通网络发送失败不会进入队列。
- API 发送队列有内置长度上限和单条保留时间，不提供 TOML 配置项；队列满时调用方需要按队满响应自行退避。
- `/protection disable` 不清空已经写入 Redis 的发送队列；重新开启保护并在微信主动对话恢复后，仍可能补发未过期的队列内容。
- 队列重放是按顺序补发，不承诺 exactly-once；少数崩溃或外部状态失败窗口可能造成重复投递，接收方仍应按消息 ID 做必要容忍。
...
- 2026-06-25：发送保护冻结期的 HTTP API 普通文本从直接锁定改为写入 Redis 发送队列并返回已入队；微信主动对话恢复后按 FIFO 补发，队列满返回队满错误，控制台普通文本仍即时拒绝；`/protection status` 增加待发队列长度，用户文档补充旧客户端从 `429` 迁移到 `202 status=queued` 的处理方式。
