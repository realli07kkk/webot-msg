...
- Install：首次安装动作，先用 Go 编译 `cmd/webot-msg` 到仓库 `bin/`，再准备 `~/.webot-msg/` 运行目录、默认 Runtime config 和 `systemd` service。
- Upgrade：升级动作，先判断 `systemd` 中是否有运行中的 `webot-msg` 实例；运行中则 stop，编译新二进制并替换，之后只在原本运行时重新 start。
- Runtime config：现有 TOML 启动配置，默认语义由 `internal/runtimeconfig` 和用户文档共同定义；部署脚本只生成默认配置文件，不新增配置 key。
- Service unit：`systemd` service 文件，使用 `ExecStart=<绝对二进制路径> -c <绝对配置路径>` 启动 `webot-msg`。注意：`systemd` 的 `ExecStart` 不能依赖 shell 展开 `~`。

## 1. 决策与约束

需求摘要：给当前项目添加 Linux 环境编译部署脚本，提供安装和升级两个核心命令。安装时编译二进制到 `bin/`，创建 `~/.webot-msg/` 及 `config/`、`logs/` 等二级目录，把默认配置文件放到对应位置，并生成 `systemd` service；升级时用 `systemd` 检查服务是否正在运行，运行中先 stop，编译最新二进制并替换，替换后只在原本运行时 start。

成功标准：
- 执行安装命令后，仓库 `bin/webot-msg` 存在并来自当前源码。
...
- 安装会生成名为 `webot-msg.service` 的 `systemd` service，`ExecStart` 使用 `-c` 指向默认配置文件的绝对路径。
...
- 二进制保留在仓库 `bin/webot-msg`。理由：用户明确要求“go 编译二进制放到 bin 目录”；service 使用该二进制的绝对路径，避免 PATH 差异。
...
- 二进制替换采用先编译到临时文件、成功后原子移动到 `bin/webot-msg` 的策略，避免编译失败时破坏旧二进制。
...
```ini
ExecStart=/abs/repo/bin/webot-msg -c /home/deploy/.webot-msg/config/webot-msg.toml
```

- 新增部署路径值对象或脚本变量：repo root、binary path、deploy user、deploy home、config path、log dir、service path。所有进入 service 文件的路径都必须是绝对路径。
...
flowchart TD
  ...
  F --> G[创建 ~/.webot-msg/config 与 logs]
  G --> H{默认 TOML 是否存在}
...
- 权限：脚本自身不要求全程 root；写 systemd unit 和执行 systemctl 时通过 `sudo`，二进制和 `~/.webot-msg/` 归部署用户所有。
...
- 默认配置文件：`~/.webot-msg/config/webot-msg.toml`，用于 service `-c` 参数。
- systemd service：`/etc/systemd/system/webot-msg.service`，用于 `systemctl` 管理进程。
- 构建产物：仓库 `bin/webot-msg`，service 的 `ExecStart` 直接引用它。
...
2. 构建与替换节点：实现 `go build` 到临时二进制，再替换 `bin/webot-msg`。
   退出信号：install/upgrade 都能生成可执行的 `bin/webot-msg`；编译失败时旧二进制不被破坏。
3. 安装节点：创建 `~/.webot-msg/config/`、`~/.webot-msg/logs/`，写入默认 TOML，生成并 reload service。
   退出信号：重复 install 不覆盖已有 TOML；service 文件 `ExecStart` 使用绝对二进制路径和绝对配置路径。
...
关键场景清单：
- 输入：在支持 `systemd` 且安装 Go 的 Linux 上执行 `./scripts/linux-service.sh install` → 期望：`bin/webot-msg` 被编译出来，`~/.webot-msg/config/` 和 `~/.webot-msg/logs/` 存在，`~/.webot-msg/config/webot-msg.toml` 存在。
...
- 输入：install 后查看 `/etc/systemd/system/webot-msg.service` → 期望：`ExecStart` 使用 `bin/webot-msg` 的绝对路径，并带 `-c /home/.../.webot-msg/config/webot-msg.toml`。
...
- 输入：服务 active 时执行 `./scripts/linux-service.sh upgrade` → 期望：脚本先 stop，编译并替换 `bin/webot-msg`，然后 start，最终服务恢复 active。
- 输入：服务 inactive 时执行 `./scripts/linux-service.sh upgrade` → 期望：脚本编译并替换 `bin/webot-msg`，最终不自动 start。
- 输入：`go build` 失败时执行 install 或 upgrade → 期望：脚本非零退出，旧的 `bin/webot-msg` 不被破坏。
