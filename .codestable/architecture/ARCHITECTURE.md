...
- 前台 console：无参运行 `webot-msg` 且没有可用 control socket 时，service 启动完整监听、HTTP API、保护和 control socket 后进入的本地交互控制台；stdin/stdout 是 TTY 时支持本地行编辑和 Tab 补全，按 Ctrl+C 会保存配置并退出进程。代码锚点：`cmd/webot-msg/main.go:18`、`internal/app/app.go:150`。
- Control console：service 通过 Unix socket 暴露的交互控制台通道，复用 `internal/console` 命令语义；无参 `webot-msg` 会在 socket 可连接时优先接入已有 service，也可用 `nc -U` / `socat` 等本地 socket 客户端连接；`/exit` 和 `/quit` 只关闭当前连接，不停止 service。代码锚点：`cmd/webot-msg/main.go:81`、`internal/control/server.go:41`。
...
- Control client helper：`internal/control` 提供 `Attach` / `AttachInteractive` / `outputSplitter` 等 client 端 helper；当前可执行入口没有单独 attach 子命令，但无参 `webot-msg` 会在启动 service 前调用这些 helper 接入已存在的 live socket。代码锚点：`cmd/webot-msg/main.go:81`、`internal/control/client.go:9`、`internal/control/interactive_client.go:26`、`internal/control/output_splitter.go:15`。
...
- 审计开关状态文件：`~/.webot-msg/state/audit.json`，只存 `audit_enabled` 布尔值，与 `protection.json` 并列、机制一致；由 service 运行时创建，部署脚本不写入。代码锚点：`internal/audit/state_store.go:16`、`internal/app/app.go:417`。
...
`cmd/webot-msg/main.go` 是唯一可执行入口，只接受无参运行。带任何参数都会向 stderr 打印一行错误并以退出码 2 结束；无参时固定读取 `~/.webot-msg/config/webot-msg.toml`，默认文件缺失时回退内置默认值，加载并校验 Runtime config，然后先尝试连接解析后的 control socket path。socket 可连接时，本进程作为 control client 接入已有 service 并在连接结束后退出；socket 不存在、拒绝连接或是 stale socket 时，才准备本地存储目录和文件日志，随后按 `[telemetry]` 配置初始化 OpenTelemetry traces，再用解析后的 auth store path、iLink BaseURL 与 control socket path 创建应用。API 端口只来自 TOML 或内置默认值，不再有 CLI 覆盖入口。代码锚点：`cmd/webot-msg/main.go:19`、`cmd/webot-msg/main.go:61`、`cmd/webot-msg/main.go:78`、`cmd/webot-msg/main.go:120`。

`internal/runtimeconfig` 是启动配置计算层。它先给出内置默认值，再按可选 TOML 覆盖，最后做 `~` 展开、端口范围、BaseURL scheme、control socket path、日志大小、telemetry endpoint/protocol 和未知 key 校验。默认存储根目录是 `~/.webot-msg/`，默认 auth store、日志路径、保护开关状态文件和控制台 socket 分别落在 `config/`、`logs/`、`state/` 和根目录；TOML 只保留 Redis 连接配置和可选 telemetry 配置，不保存保护开关值，也不暴露状态文件路径 key。旧 `[protection]` section 只作兼容解析，不再驱动运行行为。代码锚点：`internal/runtimeconfig/config.go:18`、`internal/runtimeconfig/config.go:38`、`internal/runtimeconfig/config.go:282`。

`scripts/linux-service.sh` 是 Linux/systemd 部署编排入口。`install` 会检查 Linux/systemd、Go、部署用户 home 和 sudo 权限，编译当前源码到 `bin/webot-msg`，再安装到 `/usr/local/bin/webot-msg`，创建部署用户的 `~/.webot-msg/config/` 与 `~/.webot-msg/logs/`，首次写入默认 `webot-msg.toml`，再生成 `/etc/systemd/system/webot-msg.service` 并执行 `systemctl daemon-reload`。`upgrade` 会先用 `systemctl is-active --quiet webot-msg` 记录服务是否 active；active 时先 stop，替换系统 PATH 中的二进制、刷新 systemd unit 并 `daemon-reload` 后再 start；非 active 时只替换二进制并刷新 unit，不主动启动；已有配置缺少 `[telemetry]` 或 `[redis]` 时会追加默认关闭 telemetry section 或默认 Redis section，不覆盖已有字段。`start`、`stop`、`restart`、`status` 子命令透传到 `systemctl`。代码锚点：`scripts/linux-service.sh:261`、`scripts/linux-service.sh:272`、`scripts/linux-service.sh:300`。
...
`internal/control` 提供运行中 service 的本地控制台通道。server 在配置的 Unix socket path 监听并清理 stale socket，把每个连接交给 `console.RunWithIO`；server 侧不感知 client 是否 TTY。当前可执行入口没有单独的 attach 子命令，但无参 `webot-msg` 会在启动 service 前先尝试连接 control socket：stdin/stdout 都是 TTY 时走 `AttachInteractive`，保留本地行编辑和 Tab 补全；非 TTY 时走 `Attach`，保持管道脚本的普通字节流语义。`nc -U` / `socat` 等第三方 socket 客户端仍可直接连接 control socket，按普通 line mode 输入命令，不发送协议头、不提供按键级 Tab 补全。代码锚点：`cmd/webot-msg/main.go:81`、`internal/control/server.go:17`、`internal/control/server.go:57`、`internal/control/client.go:9`、`internal/control/interactive_client.go:41`、`internal/control/output_splitter.go:15`。

`internal/ilink` 是外部 HTTP API 适配层，封装 QR 登录、拉取更新、发送消息、发送 typing 状态和 bot 配置读取。`Client` 构造时持有一个共享 `http.RoundTripper`，默认 transport 包一层 `otelhttp.NewTransport`；filter 只在请求 context 里已有有效 span 时追踪出站调用，因此 API 发送链路能串到同一 trace，后台长轮询等无入站 span 的调用不会变成手工 root span。各方法仍按原调用语义创建带各自 timeout 的 `http.Client`。所有远端请求都通过 `Client.BaseURL` 组装 endpoint。代码锚点：`internal/ilink/client.go:26`、`internal/ilink/client.go:52`、`internal/ilink/client.go:190`、`internal/ilink/client.go:373`、`internal/ilink/client.go:380`。
...
Linux deploy script 首次安装时会写入默认 Runtime config 文件 `~/.webot-msg/config/webot-msg.toml`，内容与 Runtime config 默认契约一致；已有 TOML 时不会覆盖既有字段，升级只在缺少 `[telemetry]` 或 `[redis]` section 时追加默认关闭 telemetry 配置或默认 Redis 配置；`~/.webot-msg/config/auth.json` 仍由 `config.Store` 管理，不由部署脚本迁移、覆盖或删除。保护开关状态文件由 service 运行时创建，部署脚本不直接写入。代码锚点：`scripts/linux-service.sh:167`。
...
- Telemetry 默认关闭且 TOML-only：`[telemetry]` 缺失或 endpoint 为空时不创建 exporter、不发起 OTLP 连接；启用后只按 TOML 字段决定 endpoint、protocol、headers、resource attributes 和 insecure，官方 exporter 支持的 `OTEL_EXPORTER_OTLP*` 环境变量必须被屏蔽，不能成为第二配置源。
- Telemetry 只做 traces，主要覆盖 API 入站和 iLink 出站：不引入 metrics/logs exporter，不做控制台、Redis 保护或后台长轮询的手工 span，不提供采样率配置，不引入厂商私有 SDK。唯一例外是审计 Redis 写：`audit.Record` 在 ctx 已有有效 span 时手工起一个 `audit.record` span，仍遵守“无入站 span 不产生 root span”（控制台触发的审计写不追踪），不引入 `redisotel`。
- Telemetry 启动错误快速失败，导出错误不改变业务响应：配置非法或 exporter 初始化失败走启动失败；运行期 endpoint 不可达只表现为异步导出失败，不阻塞 API 发送链路。
- systemd 交互通过本地 Unix socket，不尝试 attach systemd service 的 stdin，也不通过再启动第二个服务实例进入控制台；无参 `webot-msg` 在 socket 可连接时作为第一方 client 接入已有 service，`nc -U` / `socat` 等本地 socket 客户端保留为兼容入口。
...
- 审计复用 `[redis]` 连接、key 带 `key_prefix` 命名空间：不为审计单独配 Redis；用 `{prefix}:audit:time|body:{id}` 避免裸 UUID 污染共享 Redis。审计与保护互不影响，可独立开关。
- 审计 `Recorder` 不复用保护的 generation/refcount：审计写是单步 fire-and-forget，无跨步事务，mutex 原子换 client 即可，避免过度设计。

## 5. 代码锚点

- `cmd/webot-msg/main.go:main` — 唯一无参 CLI 入口，负责拒绝参数、加载 Runtime config、优先接入已有 control socket，必要时启动 app。
- `internal/runtimeconfig/config.go:Config` — TOML Runtime config 的结构、默认值、校验和存储准备。
- `internal/telemetry/telemetry.go:Setup` — OpenTelemetry traces 初始化入口，负责 OTLP exporter、resource、全局 TracerProvider、TraceContext propagator、env 屏蔽和 shutdown。
...
- `internal/ilink/client.go:Client` — iLink HTTP 调用封装和共享 instrumented transport。
...
- Telemetry 启动摘要只允许打印 endpoint、protocol、service_name 和 insecure，不打印 headers 或 resource attribute 值；span 属性和 resource 不得包含消息正文、BotToken、APIToken 或 ContextToken，服务端 URL 属性不保留 query string，也不启用 HTTP header 采集。代码锚点：`cmd/webot-msg/main.go:65`、`internal/api/server.go:72`、`internal/ilink/client.go:386`。
- Telemetry 配置只来自 TOML。代码允许出现 `OTEL_EXPORTER_OTLP*` 字符串的唯一目的应是屏蔽官方 exporter 的环境变量读取并恢复原环境，不得把这些环境变量转换为本项目运行配置。代码锚点：`internal/telemetry/telemetry.go:84`、`internal/telemetry/telemetry.go:111`。
...
- 部署脚本不会覆盖已有 `~/.webot-msg/config/webot-msg.toml` 的既有字段或删除 `~/.webot-msg/config/auth.json`；升级可能追加缺失的 `[telemetry]`、`[redis]` 或 `[audit]` section，追加的 telemetry endpoint 为空、audit TTL 取默认 24h，保持默认关闭。真实 Linux systemd 主机上的服务操作仍需要部署者具备 sudo 权限。代码锚点：`scripts/linux-service.sh:65`、`scripts/linux-service.sh:167`。
- 消息 ID 与审计只作用于普通文本成功发送：保护提醒、typing、保护拒绝和 reminder-only 路径不带 ID、不审计；ID 生成失败与审计写失败都 fail open，不阻断消息投递。`[audit]` 为可选 section，缺失时回退默认 24h，旧配置不会因严格 TOML 失败。代码锚点：`internal/sender/protected_text.go:87`、`internal/audit/audit.go:118`。
- 审计开关启动恢复只尝试一次：`audit.json` 记录为开启但 Redis 不可用时，服务继续启动、审计保持关闭、状态文件不改写，需修复 Redis 后手动 `/audit enable`；`audit:body` key 保存完整发送正文，敏感数据保留期由 `[audit].body_ttl` 控制。代码锚点：`internal/app/app.go:417`、`internal/audit/audit.go:134`。
