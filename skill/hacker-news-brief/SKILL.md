...
This skill is designed for CLI-only Linux hosts. Use HTTP fetches, command-line tools, and the bundled Python script; do not require a browser, Chrome extension, or desktop UI.

## Linux CLI Requirements

- Required: `python3` with the standard library.
- Useful when available: `curl` or `wget` for manual fallback source checks.
...
1. Fetch `https://news.ycombinator.com/` and collect the first 30 stories in page order.
   - Prefer running `scripts/fetch_hn_frontpage.py --limit 30` from this skill folder to avoid parsing mistakes.
